...
In 341 US county epidemic waves of winter 2020--21, the median wave reached its peak of incidence when an estimated
16\% of the population had been infected. Under a homogeneous model, the growth at the start of those waves implies
a threshold of 30\%. Infections are estimated from deaths at a fatality rate of 0.7\%, and growth is converted to a
reproduction number with a generation interval of mean 5.5 days. We call the peak of incidence the \emph{turnover}.

The gap is the herd-immunity paradox of \citet{cotton2020fundamental,cotton2020herd}. In the spring of 2020 case
counts doubled in under a week, which a homogeneous model reads as a reproduction number near 2.5 and a threshold of
60\%, yet the first waves peaked long before. Those papers traced the gap to variation in transmission across
people, places and time: early growth is dominated by the fastest transmitters, and the turnover is set by a harmonic
mean.

The mechanism is simple. People with many contacts are infected sooner, and while infectious they generate more
exposure, so each infection removes more than its share of future transmission. \citet{britton2020} and
\citet{gomes2022} quantified this for persistent differences in activity or susceptibility. \citet{tkachenko2021}
allowed activity to change over time and showed that part of the suppression is transient. They also named the
number that summarizes the effect, the \emph{immunity factor} $\lambda$: the effective reproduction number falls as
$R\,S^{\lambda}$ in the susceptible share $S$, and the turnover comes at about $1 - R^{-1/\lambda}$.

We estimate $\lambda$ in two ways that share no data. The first uses repeated contact surveys and a model of
persistent and changing activity, and never looks at an epidemic. The second backs $\lambda$ out of the peaks of the
county waves under an assumed fatality rate. The two ranges overlap, and each depends on a choice: how very high
contact counts are capped on the survey side, the fatality rate and the growth proxy on the epidemic side.
\citet{gomes2022} obtain an independent value near 3.5 by fitting heterogeneous connectivity to deaths in England and
Scotland. Where contacts were cut by lockdowns or voluntary distancing, peaks came with 1--5\% infected, and the
$\lambda$ implied by such a peak measures the cut in contacts rather than immunity.

\section{The turnover rule}

\paragraph{Definition.} In a homogeneous population the effective reproduction number is $R_e = R\,S$, with $S$ the
susceptible share. When people differ in activity, write
...
reproduction number $R^{1/\lambda}$. The factor and both relations are due to \citet[eqs.~9, 14, 15]{tkachenko2021}.

\paragraph{Scope.} For fixed gamma-distributed activity, \eqref{eq:power} is exact, as shown next. In other models we
define $\lambda$ by the initial fall of $R_e$ and use \eqref{eq:power} as an approximation to predict the turnover.
Throughout, $R$ is the initial effective reproduction number of the model. When $R$ is inferred from growth it also
depends on the assumed generation interval, and in a model with changing activity the two need not coincide.
...
\begin{equation}\label{eq:gamma}
  \lambda = 1 + 2\,\CV^2 ,
\end{equation}
as in \citet[eqs.~21--22]{gomes2022} and \citet[eq.~14]{tkachenko2021}. When only susceptibility varies the exponent
is $1 + \CV^2$.
...
converts to the overall infected share. The same formula applies to any finite type structure, and we use it below
for age crossed with activity.

\paragraph{Accuracy.} Equation \eqref{eq:lamage} is a first-order slope, so \eqref{eq:rule} is an approximation away
from the gamma case. In the joint age and activity models of Section~\ref{sec:contacts}, solved numerically to the
peak, the rule understates the infected share at the turnover by 0.6 to 1.2 percentage points at $R = 1.3$, 1.4 to
2.9 points at $R = 1.6$, and 3.6 to 7.0 points at $R = 2.5$.

\section{Activity that changes over time}\label{sec:switching}

Activity is not fixed. \citet{tkachenko2021elife} let each person's activity follow a mean-reverting square-root
process with a correlation time. A two-state version has closed forms.

\paragraph{The model.} Let each person's activity take two values with long-run shares $p_1, p_2$, mean one and
...
the stationary law $p$ and the deviation $(a - 1)\circ p$, which decays at rate $\kappa$. The second term in
\eqref{eq:growth} is the persistence of an individual's activity over the infectious period: people with more
contacts are infected sooner and, while infectious, generate more exposure, and the term contributes while a busy
phase outlasts the infection.
...
Variation across places gives the same rule as variation across people. \citet{cotton2020herd} considers a
population made of many places. To isolate the effect of heterogeneous local rates, take the places to be independent,
with equally seeded epidemics and transmission rates $\beta = \bar\beta\,\eta$, where $\eta$ has mean one across
places.

Early growth of the total reads a transmission rate $\beta_g = t^{-1}\log\mathbb{E}\,e^{t\bar\beta\eta} = G\,\bar\beta$
with $G > 1$, because the fastest places dominate the sum. The susceptible share at the turnover, averaged over
places, is $\mathbb{E}[\gamma/\beta] = J/\bar R$ with $J = \mathbb{E}[1/\eta] \ge 1$ \citep[\S\S3.1--3.2]{cotton2020herd}.
Together,
\begin{equation}
  s^\star \approx \frac{J\,G}{R_g},
\end{equation}
larger than the textbook $1/R_g$ by two convexity adjustments, one on growth and one on the turnover.

In \citet{cotton2020herd} the rate follows an Ornstein--Uhlenbeck process, the growth convexity is Vasicek's
bond-price formula, and the long-run growth excess is the variance times the correlation time
\citep[eq.~16]{cotton2020herd}. A two-state switch behaves the same way. When each place's rate switches between
$\bar\beta \pm \delta$ at rate $\nu$, the total grows at the top eigenvalue $\bar\beta - \gamma - \nu + \sqrt{\nu^2 +
\delta^2}$, which is $\bar\beta - \gamma + \delta^2/(2\nu)$ to first order. The growth convexity is again a
Green--Kubo number.
...
Within age groups the squared coefficient of variation of the number of contacts is 0.53 for ages 0--19, 0.63 for
20--64 and 0.77 for 65 and over. Combining these with the age matrices of \citet{prem2021} through \eqref{eq:lamage},
with activity held fixed, gives $\lambda = 2.70$ for the United States, 2.77 for Sweden, 2.66 for Spain, 3.02 for the
United Kingdom and 2.30 for Brazil. Age alone gives 1.3--1.7.

A one-day diary cannot tell a busy person from a busy day, so these values mix lasting and passing variation.
\citet{gomes2022}, who estimate the coefficient of variation from POLYMOD and nineteen other surveys, make the same
point and use two-day diaries to separate the two. Repeated surveys separate them further.
...
the common effect of lockdowns and seasons on each age group, though not effects that differ between people. Very
high counts are capped; the CoMix team caps at 50 contacts a day, the value they found to track changes in the
reproduction number most closely \citep{gimma2022}, and we also report caps of 20 and 100.

The covariance of a person's activity between surveys $\ell$ days apart is fitted as
\begin{equation}
...
observations at lags of a week or more cannot resolve. Table~\ref{tab:comix} gives the fit.
...
\bottomrule
\end{tabular}
\caption{Decomposition of adult activity in the CoMix UK survey.}\label{tab:comix}
\end{table}

Under this covariance model, 26--30\% of the variance of capped daily contacts is persistent and 13--14\% belongs to a
component with a correlation time of 23 to 36 days. The remaining 57--61\% is unresolved by repeat observations at
lags of a week or more. It includes day-to-day variation, and it may also include genuine correlations lasting less
than a week, which could matter over an infectious period.

The shares are similar under the three caps; the levels are not. The fitted covariance is flat from one month to the
longest lags, so the persistent component behaves like a habit over the months each person is observed. Restricting
the fit to lags of at most 200 days changes each share by at most one percentage point.
...
small. The level depends on the cap, and how many of a very busy person's contacts can transmit is unresolved.
...
deaths \citep{nyt2021}, February 2020 to June 2021. The data count deaths by the date they were reported, which adds
noise to the timing of death peaks. For each wave, early growth $r$ gives $R$ through the relation $R = 1/M(-r)$ of
\citet{wallinga2007}, with $M$ the moment generating function of an assumed gamma generation interval of mean 5.5
days and standard deviation 2.1 days. Cumulative deaths divided by an infection fatality rate give the infected share
when the wave began, $a_0$, and at the death peak, $a_1$.
...
county data identify the level of $\hat\lambda$ but not its dependence on growth, which Table~\ref{tab:predict} says
is small.
...
The two estimates overlap. Contact surveys predict an immunity factor of 4.6--5.2 at the survey's own cap of 50
contacts a day, 3.4--3.7 at 20 and 5.9--6.8 at 100. Winter county waves imply a peak index of 2.4--5.3 over fatality
rates from 0.5\% to 1\%, and \citet{gomes2022} fit about 3.5 to England and Scotland. At $R = 2.5$, $\lambda = 3.5$
puts the turnover near 23\% and $\lambda = 5$ near 17\%.

Each side carries an uncertainty of similar size. On the survey side the level depends on the cap and on the residual
below one week. On the epidemic side it scales with the assumed fatality rate and the growth proxy.
...
reduction of the threshold with a transient on top. \citet{tkachenko2021} reach a different balance: they put the
long-run immunity factor near 2 and conclude that most of the first-wave suppression was transient. A survey that
follows the same people for years would settle how long the persistent component lasts.
