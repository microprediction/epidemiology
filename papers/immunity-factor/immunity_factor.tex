...
population infected, far below the herd-immunity threshold of 60\% that the growth implies. Because the most
active people are infected first, the effective reproduction number falls faster than the susceptible share $S$:
as $R\,S^{\lambda}$ rather than $R\,S$, where the immunity factor $\lambda$ is one in a homogeneous population. The
turnover is then $1 - R^{-1/\lambda}$. We measure $\lambda$ from a contact panel that follows the same people for two years, which separates
